...
public class GeneticAlgorithm {
     ...
     * 种群规模 128
     */
    public static int entitysize = 2 << 6;
    /**
     * 变异概率
     */
    ...
    private List<WeightIndividual> initIndividuals() {
        ...
        for (int i = 0; i < list.size() - 1; i++) {
            if (weightIndividuals.size() >= entitysize) break;
    ...
    private double update_fitness(Map<WeightIndividual, List<Gameplayer>> listMap) {
        ...
        // 根据比分排序倒序 保留最优基因
        this.weightIndividuals.sort((o1, o2) -> (int) ((o2.getFitness() - o1.getFitness()) * 100));
        return all_score;
    }
    ...
    private void chooseSample(List<WeightIndividual> weightIndividuals) {
         ...
         * 下一代种群
         */
        Set<WeightIndividual> newIndividuals = new LinkedHashSet<>();
        // 最优基因不进行轮盘 直接保留
    ...
    private void checkRepeat(List<WeightIndividual> individuals, List<WeightIndividual> recom) {
        recom.removeIf(individuals::contains);
    }
